...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- LÓGICA DE SALVAR/CARREGAR/LIMPAR PROGRESSO (ATUALIZADA) ---
    ...
    document.getElementById('btn-suggest-roteiro').addEventListener('click', () => { const suggested = ['Praia do Julião (Sul)', 'Cachoeira dos Três Tombos (Sul)', 'Passeio na Vila (Centro)', 'Mirante do Coração (Sul)', 'Praia do Perequê (Centro)', 'Mergulho Ilha das Cabras (Sul)', 'Pôr do Sol no Mirante do Piúva']; document.querySelectorAll('.activity-item input').forEach(input => { input.checked = suggested.includes(input.value); }); generateRoteiro(suggested); showToast('Um roteiro de 3 dias foi sugerido!', 'info'); });
